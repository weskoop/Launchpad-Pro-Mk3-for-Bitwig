 ...
 */
class RecordArmLayer extends Layer {
  activate() {
    Layer.maybeSetSessionLayout();
    ext.buttons.setAllTrackButtonColours(Colours.RecordArmDim, Colours.RecordArm, this.getOrientation());
    this.selectTrackMode(Button.RecordArm);
  ...
  onTrackPush(idx: number) {
    ...
    ext.trackBank.getItemAt(idx).arm().toggle();
  }
}