 ...
 */
class MuteLayer extends Layer {
  activate() {
    Layer.maybeSetSessionLayout();
    ext.buttons.setAllTrackButtonColours(Colours.MuteDim, Colours.Mute, this.getOrientation());
    this.selectTrackMode(Button.Mute);
  ...
  onTrackPush(idx: number) {
    ...
    ext.trackBank.getItemAt(idx).mute().toggle();
  }
}